...
%% main function
function MIBIdataNorm2 = MIBIboundary_compensation_boundarySA(newLmod,data,countsNoNoiseRec,channelNormIdentity,elementShape,elementSize)
%default values for the method:
%elementShape = 1, Sudoku type, = 2, cross type.
if isempty(elementShape)
    elementShape = 2;
end
...
compenChannelNum = length(channelNormIdentity);
rev_channelNormIdentity = ones(compenChannelNum,1) - channelNormIdentity;
    ...
    for j = 1:length(tempRow)
        if elementSize == 1
            if (1<tempRow(j))&&(tempRow(j)<rowNum)&&(1<tempCol(j))&&(tempCol(j)<colNum)
                ...
                elseif elementShape == 2
                    %M2, cross style, 3x3 4 pixels considered
        ...
        elseif elementSize == 2
            if (2<tempRow(j))&&(tempRow(j)<rowNum-1)&&(2<tempCol(j))&&(tempCol(j)<colNum-1)
                ...
                elseif elementShape == 2
                    %M4, cross style, 5x5 12 pixels considered
